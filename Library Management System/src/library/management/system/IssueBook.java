...
public class IssueBook extends JFrame implements ActionListener {
    ...
    private final JTextField student_id, student_name, father_name, faculty, department, session, year, semester, date_of_issue;
    private final JButton bookSearchButton, studentSearchButton, issueButton, backButton;
...
//        System.out.println(x_start + " " + y_start);
        ...
        createJLabel(doi, x_start, y_start + 9 * y_gap, 80, field_width);

        date_of_issue = new JTextField();
        createJTextField(date_of_issue, x_start + label_length, y_start + 9 * y_gap, field_length, field_width);
        date_of_issue.setText(getTodaysDate());
        date_of_issue.setHorizontalAlignment(SwingConstants.CENTER);

        issueButton = new JButton("Issue");
        createButton(issueButton, x_start + 2 * label_length + field_length, y_start + 9 * y_gap, label_length, field_width);
    ...
    private String getTodaysDate() {
        ...
        String todaysDate = date.toLocaleString().substring(0, 12);
        return todaysDate;
    }
        ...
        try {
            ...
            if (source.equals(bookSearchButton)) {
                ...
                if (book_id.getText().length() == 0) {
                    JOptionPane.showMessageDialog(this, "Enter book id", "Issue Book", JOptionPane.ERROR_MESSAGE);
                } else if (book_id.getText().length() != 4) {
                    JOptionPane.showMessageDialog(this, "Book id must be a 4 digit number", "Issue Book", JOptionPane.ERROR_MESSAGE);
                    resetBookField();
                } else {
                    ...
                    if (rs.next()) {
                        ...
                        JOptionPane.showMessageDialog(this, "Book not found", "Issue Book", JOptionPane.ERROR_MESSAGE);
                        resetBookField();
                    }
                    rs.close();
                    st.close();
                ...
                if (student_id.getText().length() == 0) {
                    JOptionPane.showMessageDialog(this, "Enter student id", "Issue Book", JOptionPane.ERROR_MESSAGE);
                } else if (student_id.getText().length() != 8) {
                    JOptionPane.showMessageDialog(this, "Student id must be a 8 digit number", "Issue Book", JOptionPane.ERROR_MESSAGE);
                    resetStudentField();
                } else {
                    ...
                    if (rs.next()) {
                        ...
                        JOptionPane.showMessageDialog(this, "Student not found", "Issue Book", JOptionPane.ERROR_MESSAGE);
                        resetStudentField();
                    }
                    st.close();
                    rs.close();
            ...
            } else if (source.equals(issueButton)) {

                if (book_id.getText().length() == 0 || student_id.getText().length() == 0) {
                    JOptionPane.showMessageDialog(this, "Search book and student first", "Issue Book", JOptionPane.ERROR_MESSAGE);
                    ...
                    if (r1.next()) {
                        JOptionPane.showMessageDialog(this, "Book already issued", "Issue Book", JOptionPane.INFORMATION_MESSAGE);
                        resetBookField();
                        resetStudentField();

                    } else {
                        String sql2 = "insert into issue_book (book_id, student_id, date_of_issue) values(?, ?, ?);";
                        PreparedStatement st2 = con.c.prepareStatement(sql2);
                        st2.setInt(1, b_id);
                        st2.setInt(2, stu_id);
                        st2.setString(3, date_of_issue.getText());
                        ...
                        if (r2 == 1) {
                            ...
                            JOptionPane.showMessageDialog(this, "Couldn't connect to the server.", "Issue Book", JOptionPane.ERROR_MESSAGE);
                        }
                    }
                    r1.close();
                    st1.close();
            ...
            JOptionPane.showMessageDialog(this, "Book Id and Student Id must be number", "Add Book", JOptionPane.ERROR_MESSAGE);
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, "Couldn't connect to the server\nBook or Student could not searched.", "Add Book", JOptionPane.ERROR_MESSAGE);
        }
    }
